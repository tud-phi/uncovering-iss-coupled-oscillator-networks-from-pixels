...
> <cite>R1</cite>: Theorem 1 seems to be trivial as it elaborates on the (nonlinear) coupling of passive systems (damped mass-spring systems), which is always passive since it does not â€œproduceâ€ any energy and thus, is inherently stable. (Same for Theorem 2). In this sense, the theoretical contribution seems limited. Is there anything I missed?

We thank the Reviewer for their question and for allowing us to further elaborate on this topic. In short, we differ with the ReviewerÕs technical arguments, and Ð consequently Ð with their conclusions on the triviality of Th.1. 
In essence, we recognize that the ReviewerÕs arguments are well funded in the context of port-Hamiltonian theory. However, substantial extra properties/assumptions are needed to yield to the ReviewerÕs conclusion. Thus, the PH way of proving Th.1 would be an interesting and proper one, but it would definitely not result in the trivial proof the Reviewer is hinting at.
...
Passivity, even in the scalar case, does not imply (global) stability. Additional assumptions on the (global) convexity of the energy landscape are needed Ð which would be hard to impose.  As a trivial counter example, think of a damped mass placed on the top of an infinite hill. 
...
We study a scalar CON with $K=-1$, positive damping $D=0.4$, and $W = 3$, $b=0$ with the EOM $\ddot{x} + K x + D \dot{x} = \tau$. [MOU1]We take the passive output $o = \dot{x}$, and we prove passivity according to Def. 6.3 (Khalil, 2001) using the storage function $V(x) = K x^2 + \dot{x}^2$: $\dot{V}(x, \tau) = \tau \dot{x} - D \dot{x}^2 \leq \tau o. $
...
where $k=1$, $d=0.4$, and $w=3$ are globally asymptotically stable. 

However, the linear stiffness matrix is negative definite,
and therefore, the system is not globally asymptotically stable. This is illustrated in Fig. R3 of the global response.


### PID vs. PD

> <cite>R1</cite>: The statement in Line 266 â€œPID controller has several well-known drawbacks, such [â€¦] steady-state errors (in case the integral gain is chosen to be zero)â€ is misleading as in this case we would say itâ€™s a PD controller.

We thank the Reviewer for their comment and for pointing out the mistake. [MOU2]We agree that this sentence is indeed badly written and confusing.
...
For this rebuttal, we have simulated a _P-satI-D + FF_ controller with $K_\mathrm{p} = 0$, $K_\mathrm{i} = 2$, and $K_\mathrm{d} = 0.1$, which means that we set the proportional term to zero and increased the damping factor.[MOU4]
...
To make this focus clear, we will, in the final paper, modify the sentence starting on line 69 to (with changes marked in **bold**):[MOU5]
...
[MOU1]Where is W?
[MOU2]Good, but cconsider removing/shorten these if you need more space
[MOU3]We should mention that - thanks to the two proofs that we provide - the network allows to impose a beneficial inductive bias interms of conserving global stability and ISS. This is an important aspect from the model point of you.

In saying this, we should also stress that the proofs are a contribution too.
[MOU4]In my opinion, this can be shortened as it does not contribute much to answering their question. 

To answer to their question we just need to say: 
1 - due to poorly tuned
2 - we did that to have direct comparison with the alternatives
3 - we can re-tune to get better performance
4 - we report them in the novel version of the paper
[MOU5]Good. You could also add a sentence in the conclusions saying something like ÒThis work was specifically focused on how to impose structural biases that enforce second order Lagrangian structure and stability properties. However, we believe that the propsoed model can be used more broadly, beyond mechanical systems, whenever strong stability guarantees are needed. This will be the focus of future research. Ó
