...
def task_factory(nn_model: nn.Module, loss_weights: Dict[str, float] = None) -> TaskCallables:
    """
    ...
    def predict_fn(batch: Dict[str, Array], nn_params: FrozenDict) -> Dict[str, Array]:
        ...
        q_pred_bt = nn_model.apply({"params": nn_params}, img_bt, method=nn_model.encode)
        # output will be of shape batch_dim * time_dim x width x height x channels
        img_pred_bt = nn_model.apply({"params": nn_params}, q_pred_bt, method=nn_model.decode)
    ...
    def compute_metrics(batch: Dict[str, Array], preds: Dict[str, Array]) -> Dict[str, Array]:
        ...
        metrics = {
            ...
            "mse_rec": jnp.sqrt(jnp.mean(jnp.square(preds["rendering_ts"] - batch["rendering_ts"])))
        }
        return metrics
